...
class ProfileController extends BaseController
{
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->memberModel->update($userId, $postData)) {
                ...
                $this->auditModel->log(
                    ...
                    'sp_members',
                    $userId,
                    'Member updated their profile',
                    $member,
                    $postData
                );

                return redirect()->to(base_url('member/profile'))
    ...
    public function employment()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->memberModel->update($userId, $postData)) {
                ...
                $this->auditModel->log(
                    ...
                    'sp_members',
                    $userId,
                    'Member updated employment information'
                );

                return redirect()->to(base_url('member/profile'))
                    ->with('success', 'Informasi pekerjaan berhasil diperbarui');
            }

            return redirect()->back()
                ->withInput()
                ->with('error', 'Gagal memperbarui informasi pekerjaan');
        }
        ...
        return view('member/profile/employment', $data);
    }
